    ...
    if (ID !== 0) {
        if (FUNCTION.name === "ex_models" && Array.isArray(ID)) {
            ...
            if (temp_index === null) {
                ...
                if (IS_APPEND) {
                    su32(OFFSET, temp_offset, is_little_endian)
                } else {
                    su32(OFFSET, temp_offset - g.m, is_little_endian)
                }
                if (CUSTOM === "model") {
...
function ex_find(n, y) {
    let html = `bad val: ${n} -> ${n + g.m}\n  good val: ${y} -> ${y + g.m}`
    console.log(html)
}

function su8(o, v) {
    new DataView(dynamic_buffer).setUint8(o, v, is_little_endian)
}
function su16(o, v) {
    new DataView(dynamic_buffer).setUint16(o, v, is_little_endian)
}
function su32(o, v) {
    new DataView(dynamic_buffer).setUint32(o, v, is_little_endian)
}
function sf32(o, v) {
    new DataView(dynamic_buffer).setFloat32(o, v, is_little_endian)
}

function ex_byte_alignment_testing(o) {
    e = o

    if (g.file_name === 'Intrface.xpc') {
        if (o === 41322964) {
            e += 64
        } else if (o === 41322836) {
            e += 16
    ...
    } else if (g.file_name === 'loading.xpc') {
        if (o === 456) {
            e += 192
        } else if (o === 11036) {
            e += 576
