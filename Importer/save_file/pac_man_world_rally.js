#! for the pac man world rally save file format.
function save_file_import(file_name) {
...
function save_value_changed() {
    ...
    if (temp_this.title === 'display') {
        ...
        new DataView(buffer).setUint32(temp_this.dataset.offset, checked, g.endian)

    } else if (temp_this.title === "time") {

        new DataView(buffer).setUint32(temp_this.dataset.offset, temp_this.value, g.endian);

        temp_string = u32(temp_this.dataset.offset, g.endian)
...
function save_file_ghost_import(file_name) {
