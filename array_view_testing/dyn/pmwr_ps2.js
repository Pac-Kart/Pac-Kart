...
function im_pmwr_ps2_file_header(o, i, x) {
    ...
    for (let i = 0; i < u32(12); i++) {
        let a = Date.now()
        im_pmwr_ps2_directory(16 + (i * 24), i, x[i].directory, directory_offset)
...
function im_pmwr_ps2_directory(o, i, x, global) {
    ...
    u32(o + 16) ? im_pmwr_ps2_datapack(next_offset + u32(o + 20), 0, x[i].section_datapack) : 0;
...
function ex_pmwr_ps2_file_header(o, x) {
    ...
    for (let i = 0; i < x[0].directory.length; i++) {
        ...
        e = ex_pmwr_ps2_directory(16 + (i * 24), e, x[0].directory[i], global)
        time_array.push(Date.now() - a)
...
function ex_pmwr_ps2_directory(o, e, x, global) {
    ...
    e = ex_pmwr_ps2_datapack(16 + (i * 24), e, x[0].section_datapack[i], global)
